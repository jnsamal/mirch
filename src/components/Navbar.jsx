...
/* ---------------------------------------------------------
   Navbar — standard horizontal bar, fixed to top
--------------------------------------------------------- */
export default function Navbar({ onOrder }) {
  ...
  return (
    <div className="fixed z-40 top-0 left-0 right-0">
      <Glass
        className="rounded-none px-5 sm:px-8 md:px-14 py-4 flex items-center justify-between"
        style={{ background: C.ink, border: `1px solid ${C.ink}`, borderTop: "none", color: "#fff" }}
      >
      ...
      {open && (
        <Glass
          className="md:hidden px-5 sm:px-8 md:px-14 py-4 flex flex-col gap-4"
          style={{ background: C.ink, border: `1px solid ${C.ink}`, borderTop: "none", color: "#fff" }}
        >
